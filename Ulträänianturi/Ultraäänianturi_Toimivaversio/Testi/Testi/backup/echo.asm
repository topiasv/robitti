...
;  Global Symbols
;-----------------------------------------------
export  Echo_EnableInt
export _Echo_EnableInt
export  Echo_DisableInt
export _Echo_DisableInt
export  Echo_Start
export _Echo_Start
export  Echo_Stop
export _Echo_Stop
export  Echo_WritePeriod
export _Echo_WritePeriod
export  Echo_WriteCompareValue
export _Echo_WriteCompareValue
export  Echo_wReadCompareValue
export _Echo_wReadCompareValue
export  Echo_wReadCounter
export _Echo_wReadCounter

; The following functions are deprecated and subject to omission in future releases
;
export  wEcho_ReadCompareValue  ; deprecated
export _wEcho_ReadCompareValue  ; deprecated
export  wEcho_ReadCounter       ; deprecated
export _wEcho_ReadCounter       ; deprecated
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Echo_WritePeriod
;
;  DESCRIPTION:
...
_Echo_WritePeriod:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[Echo_PERIOD_LSB_REG], A
   mov   A, X
   mov   reg[Echo_PERIOD_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Echo_WriteCompareValue
;
;  DESCRIPTION:
...
;    functions.
;
 Echo_WriteCompareValue:
_Echo_WriteCompareValue:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[Echo_COMPARE_LSB_REG], A
   mov   A, X
   mov   reg[Echo_COMPARE_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Echo_wReadCompareValue
;
;  DESCRIPTION:
...
;    functions.
;
 Echo_wReadCompareValue:
_Echo_wReadCompareValue:
 wEcho_ReadCompareValue:                         ; this name deprecated
_wEcho_ReadCompareValue:                         ; this name deprecated
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   A, reg[Echo_COMPARE_MSB_REG]
   mov   X, A
   mov   A, reg[Echo_COMPARE_LSB_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Echo_wReadCounter
;
;  DESCRIPTION:
...
;
 Echo_wReadCounter:
_Echo_wReadCounter:
 wEcho_ReadCounter:                              ; this name deprecated
_wEcho_ReadCounter:                              ; this name deprecated
   ...
   mov   A, reg[Echo_COMPARE_MSB_REG]            ; Save the Compare register on the stack
   push  A                                          ;
   mov   A, reg[Echo_COMPARE_LSB_REG]            ;
   push  A                                          ;   -stack frame now 2 bytes-
   mov   A, reg[Echo_CONTROL_LSB_REG]            ; Save CR0 (running or stopped state)
   push  A                                          ;   -stack frame now 3 bytes-
   Echo_Stop_M                                   ; Disable (stop) the Counter if running
   M8C_SetBank1                                     ;
   mov   A, reg[Echo_INPUT_LSB_REG]              ; save the LSB clock input setting
   push  A                                          ;   on the stack (now 4 bytes) and ...
                                                    ;   hold the clock low:
   mov   reg[Echo_INPUT_LSB_REG], INPUT_REG_NULL
   M8C_SetBank0                                     ; Extract the Count via DR2 register
   mov   A, reg[Echo_COUNTER_MSB_REG]            ; DR2 <- DR0 (in the MSB block)
   mov   A, reg[Echo_COMPARE_MSB_REG]            ; Stash the Count MSB on the stack
   push  A                                          ;   -stack frame is now 5 bytes
   mov   A, reg[Echo_COUNTER_LSB_REG]            ; DR2 <- DR0 (in the LSB block)
   mov   A, reg[Echo_COMPARE_LSB_REG]            ; Stash the Count LSB on the stack
   ...
   mov   reg[Echo_INPUT_LSB_REG], A              ;   and restore it
   M8C_SetBank0                                     ; Now re-enable (start) the counter
   mov   A, [X+bOrigControlReg]                     ;   if it was running when
   mov   reg[Echo_CONTROL_LSB_REG], A            ;   this function was first called
